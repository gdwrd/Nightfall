...
describe('parseIndex', () => {
  ...
  it('handles dash separator as well as em-dash', () => {
    const content = [
      '# Memory Index',
      '- project.md - project goals with dash',
    ].join('\n')
...
describe('serializeIndex', () => {
  it('serializes a MemoryIndex back to markdown', () => {
    const index = {
      ...
      components: [
        { file: 'components/auth.md', description: 'auth flow' },
      ],
    }
...
describe('initializeMemoryBank', () => {
  it('creates correct directory structure on a sample project', async () => {
    ...
    fs.writeFileSync(path.join(projectRoot, 'src', 'auth', 'login.ts'), 'export function login() {}')
