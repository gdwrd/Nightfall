...
function validateConfig(config: NightfallConfig): void {
  ...
  if (typeof provider.port !== 'number' || provider.port <= 0 || provider.port > 65535) {
    throw new Error('Config validation failed: provider.port must be a valid port number (1-65535)')
  }
...
export async function loadConfig(): Promise<NightfallConfig> {
  ...
  const merged = deepMerge(DEFAULT_CONFIG as unknown as Record<string, unknown>, userConfig as Record<string, unknown>) as unknown as NightfallConfig
